...
%% Select data files %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
path = '../triangle_measurements/';     % Specifiy the path to the folder where the measurement data are stored

meas_name = 'measurement_H_fast_x.mat';  % file name of the measurement data, x-axis
% meas_name = 'measurement_H_fast_y.mat';  % y-axis
% meas_name = 'measurement_H_fast_z.mat';  % z-axis
...
input_path = path;
input_name = 'input_H_fast.mat';         % file name for the input data
...
doSaveGIRFs = 1;                % Set to 1 if you want to save the result of the calculation, otherwise set to 0
...
prep.numTriang = 1;             % number of triangles per delay used in the measurement
prep.numTriangs4fft = 2;        % number of triangles to use for the calculation of H_FFT
prep.numDelays = 7;             % number of delays used in the measurement
...
% Specify (if needed) which data points to exclude from the calculation
prep.discardData{1}(:,1,1000:end,1:2) = 0;      % We exclude the last 30 ms of the measurements
                                                % in which the excitation precedes the triangles
                                                % because this time window is also covered in the
                                                % measurements with a delay of 1 ms between triangles
                                                % and excitation.
disp('    Data prepared.')
...
plotter.plot_GSTFs(H_fft,['H^f^a^s^t_F_F_T_,_',ax_name], H_LF,['H^f^a^s^t_L_F_,_',ax_name], H_HF,['H^f^a^s^t_H_F_,_',ax_name], H_combined,['H^f^a^s^t_',ax_name], 4, term2plot, ax_name);
disp('    main_H_fast.m finished.')
